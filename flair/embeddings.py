...
class ELMoEmbeddings(TokenEmbeddings):
    ...
    def __init__(
        self, model: str = "original", options_file: str = None, weight_file: str = None, embedding_name = None, is_hit_elmo = False, use_avg = False,
    ):
        super().__init__()
        ...
        self.is_hit_elmo = is_hit_elmo
        if self.is_hit_elmo:
            from ELMoForManyLangs.elmoformanylangs import Embedder
            self.ee = Embedder(model)
        else:
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]) -> List[Sentence]:
        ...
        if self.is_hit_elmo:
            embeddings = self.ee.sents2elmo(sentence_words,-2)
        else:
...
class TransformerWordEmbeddings(TokenEmbeddings):
    def __init__(
        ...
        **kwargs
        ...
        self.sentence_feat = sentence_feat
        self.use_internal_doc = use_internal_doc
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]) -> List[Sentence]:
        ...
        if hasattr(self,'v2_doc') and self.v2_doc:
            ...
            for sentence in sentences:
                ...
                if self.name not in sentence.token_subtoken_lengths:
                    tokenized_string = sentence.to_tokenized_string()
                    subtokenized_sentence = self.tokenizer.tokenize(tokenized_string)
                    sentence.subtokenized_sentence[self.name] = subtokenized_sentence
                else:
                    subtokenized_sentence = sentence.subtokenized_sentence[self.name]
                if len(subtokenized_sentence) > self.max_subtokens_sequence_length:
                    single_sentences.append(sentence)
                else:
                    doc_sentences.append(sentence)
            if len(doc_sentences)>0:
                self.add_document_embeddings_v2(doc_sentences, max_sequence_length = model_max_length, batch_size = 32 if not hasattr(self,'doc_batch_size') else self.doc_batch_size)
            if len(single_sentences)>0:
                self._add_embeddings_to_sentences(single_sentences)
    ...
    def _add_embeddings_to_sentences(self, sentences: List[Sentence]):
        ...
        if hasattr(self, 'use_internal_doc') and self.use_internal_doc:
            sentences = [sentence.doc_sent for sentence in sentences]
        ...
        with gradient_context:
            if hasattr(self, 'use_internal_doc') and self.use_internal_doc:
                final_sentences = input_sentences
            else:
                final_sentences = sentences
            # iterate over all subtokenized sentences
            for sentence_idx, (sentence, subtoken_lengths, nr_sentence_parts) in enumerate(zip(final_sentences, subtokenized_sentences_token_lengths, sentence_parts_lengths)):
