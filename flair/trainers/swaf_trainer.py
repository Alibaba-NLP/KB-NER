...
class SWAFTrainer(ModelDistiller):
	def __init__(
		self,
	...
	def train(
		...
		**kwargs,
		...
		batch_loader=ColumnDataLoader(list(self.corpus.dev),mini_batch_size,shuffle,use_bert=self.use_bert,tokenizer=self.bert_tokenizer, sort_data=sort_data, model = self.model, sentence_level_batch = self.sentence_level_batch)
